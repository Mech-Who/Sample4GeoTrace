...
class CVUSADatasetTrain(Dataset):
    def __init__(self,
        ...
        self.train_file = os.path.join(data_folder, 'splits', 'train-19zl.csv')
        self.pairs = self.read_csv(self.train_file)

        # self.idx2pair = {idx: pair for idx, pair in enumerate(self.pairs)}
        # self.samples = list(range(len(self.pairs)))
        # self.train_ids = list(range(len(self.pairs)))
    ...
    def read_csv(self, file_path):
        ...
        with open(file_path, 'r') as f:
            ...
            for line in lines:
                ...
                ground = items[1]
                ground_anno = items[2]
                idx = int(sat.split('/')[-1].split('.')[0])
                pairs.append((idx, sat, ground))
        return pairs
    ...
    def __getitem__(self, index):
        ...
        # load query -> ground image
        query_img = cv2.imread(os.path.join(self.data_folder, ground))
        query_img = cv2.cvtColor(query_img, cv2.COLOR_BGR2RGB)

        # load reference -> satellite image
        reference_img = cv2.imread(os.path.join(self.data_folder, sat))
        reference_img = cv2.cvtColor(reference_img, cv2.COLOR_BGR2RGB)
...
class CVUSADatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        if split == 'train':
            self.csv_file = f'{data_folder}/splits/train-19zl.csv'
        else:
            self.csv_file = f'{data_folder}/splits/val-19zl.csv'
    ...
    def _load_data(self):
        with open(self.csv_file, 'r') as f:
            ...
            for row in reader:
                sat_image = row[0]
                ground_image = row[1]
                idx = int(sat_image.split('/')[-1].split('.')[0])
