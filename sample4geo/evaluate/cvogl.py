...
def evaluate(config,
    ...
    print("\nExtract Features:")
    query_features, ref_features, scale1, scale2, click_xy, gt_box = predict(config, model, dataloader) 
    
    print("Compute Scores:")
    score =  calculate_scores(config, query_features, ref_features, scale1, scale2, click_xy, gt_box) 
...
def calculate_scores(config, query_feature, ref_feature, scale1, scale2, click_xy, gt_box):
    ...
    top_k_patches, index_hw = find_most_similar_k_patch(query_patch, target_patches, 5) # [batch_size, k, channels, patch_size, patch_size], [batch_size, k, 2]
    ...
    # 计算得分
    _, k, _ = bbox.shape
    iou = bbox_iou(gt_box.repeat(1, k).reshape(-1, 4), bbox.reshape(-1, 4), x1y1x2y2=False)
...
def similarity(patch1, patch2):
    sim = F.cosine_similarity(patch1.reshape(patch1.shape[0], -1), patch2.reshape(patch2.shape[0], -1), dim=1)
